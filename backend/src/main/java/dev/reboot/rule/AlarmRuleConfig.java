...
public class AlarmRuleConfig {
    ...
    public List<AlarmRule> alarmRules() {
        return List.of(
            // ── 温度 ──
            new AlarmRule("TEMPERATURE", Operator.GT,  "40.0",  2, "OVER_TEMP",
                    "设备 {deviceId} 温度过高: {value}°C (阈值 {threshold}°C)"),
            new AlarmRule("TEMPERATURE", Operator.LT,  "0.0",   2, "UNDER_TEMP",
                    "设备 {deviceId} 温度过低: {value}°C (阈值 {threshold}°C)"),

            // ── 压力 ──
            new AlarmRule("PRESSURE", Operator.GT,  "110.0", 1, "OVER_PRESSURE",
                    "设备 {deviceId} 压力过高: {value}kPa (阈值 {threshold}kPa)"),
            new AlarmRule("PRESSURE", Operator.LT,  "90.0",  2, "UNDER_PRESSURE",
                    "设备 {deviceId} 压力过低: {value}kPa (阈值 {threshold}kPa)"),

            // ── 转速 ──
            new AlarmRule("SPEED", Operator.GT,  "3000.0", 3, "OVER_SPEED",
                    "设备 {deviceId} 转速过高: {value}RPM (阈值 {threshold}RPM)"),
            new AlarmRule("SPEED", Operator.LT,  "100.0",  2, "UNDER_SPEED",
                    "设备 {deviceId} 转速异常偏低: {value}RPM (阈值 {threshold}RPM)"),

            // ── 湿度 ──
            new AlarmRule("HUMIDITY", Operator.GT,  "90.0",  1, "OVER_HUMIDITY",
                    "设备 {deviceId} 湿度过高: {value}% (阈值 {threshold}%)"),
            new AlarmRule("HUMIDITY", Operator.LT,  "10.0",  1, "UNDER_HUMIDITY",
