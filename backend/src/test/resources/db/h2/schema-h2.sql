...
-- 与正式迁移 V1~V7 的关系:
--   - 列/类型/约束/唯一键镜像 V1+V3+V4+V5+V6+V7；
...
CREATE TABLE IF NOT EXISTS `user` (
    ...
    `failed_attempts` INT        NOT NULL DEFAULT 0       COMMENT '连续登录失败次数（V5）',
    `locked_until`  DATETIME     DEFAULT NULL             COMMENT '锁定截止时间（V5）',
    `password_changed_at` DATETIME DEFAULT NULL          COMMENT '最近改密时间（V5）',
...
CREATE TABLE IF NOT EXISTS `role` (
    ...
    `status`       TINYINT      NOT NULL DEFAULT 1       COMMENT '状态：1-启用 0-禁用（V7）',
    `is_deleted`   TINYINT      NOT NULL DEFAULT 0       COMMENT '逻辑删除（V7）',
    `created_at`   DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`   DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间（V7）',
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `device` (
    `id`           BIGINT       NOT NULL AUTO_INCREMENT  COMMENT '主键',
    `site_id`      BIGINT       NOT NULL DEFAULT 1       COMMENT '站点 ID（V4）',
    ...
    `updated_at`   DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_device_code_deleted` (`device_code`, `is_deleted`),
    KEY `idx_device_type` (`device_type`),
    KEY `idx_device_status` (`status`),
    KEY `idx_device_is_deleted` (`is_deleted`),
    KEY `idx_device_site_id` (`site_id`)
...
CREATE TABLE IF NOT EXISTS `alarm` (
    ...
    `acknowledged_at` DATETIME    DEFAULT NULL             COMMENT '确认时间（V7）',
    `acknowledged_by` BIGINT      DEFAULT NULL             COMMENT '确认人 ID（V7）',
    `resolved_by`   BIGINT        DEFAULT NULL             COMMENT '解决人 ID（V7）',
    `created_at`    DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`    DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间（V7）',
    PRIMARY KEY (`id`),
    ...
    KEY `idx_alarm_acknowledged_by` (`acknowledged_by`)
...
CREATE TABLE IF NOT EXISTS `operation_log` (
    ...
    CONSTRAINT `chk_operation_type` CHECK (`operation_type` IN ('CREATE','UPDATE','DELETE','LOGIN','EXPORT','ACKNOWLEDGE','RESOLVE','CHAT','SUMMARY','DIAGNOSE','FUNCTION_CALL','INGEST')),
    `target_type`     VARCHAR(32)   NOT NULL                 COMMENT '目标类型',
    CONSTRAINT `chk_target_type` CHECK (`target_type` IN ('USER','DEVICE','ALARM','ROLE','AI','KNOWLEDGE')),
...
CREATE TABLE IF NOT EXISTS `login_audit` (
    `id`           BIGINT       NOT NULL AUTO_INCREMENT  COMMENT '主键',
    `user_id`      BIGINT       DEFAULT NULL             COMMENT '用户 ID（V6）',
    `username`     VARCHAR(64)  NOT NULL                 COMMENT '用户名',
