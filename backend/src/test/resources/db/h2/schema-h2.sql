...
CREATE TABLE IF NOT EXISTS `operation_log` (
    ...
    CONSTRAINT `chk_operation_type` CHECK (`operation_type` IN ('CREATE','UPDATE','DELETE','LOGIN','EXPORT','ACKNOWLEDGE','RESOLVE','CHAT','SUMMARY','DIAGNOSE','FUNCTION_CALL')),
    `target_type`     VARCHAR(32)   NOT NULL                 COMMENT '目标类型',
    CONSTRAINT `chk_target_type` CHECK (`target_type` IN ('USER','DEVICE','ALARM','ROLE','AI')),
